...
两个 schema 共用 graphql-common 的请求预算：深度 8、加权复杂度 200,000、展开字段 256、
根字段 20。分页按 pageSize 计权，无分页列表使用 100 的估算权重，抓取入口附加成本 1,000。
...
不调用数据库或爬虫。共享测试支持位于 `common/graphql-common/test-support`。
