/*
 * @Author: suxiaoshao suxiaoshao@gmail.com
 * @Date: 2024-01-06 01:30:13
 * @LastEditors: suxiaoshao suxiaoshao@gmail.com
 * @LastEditTime: 2024-03-28 09:34:17
 * @FilePath: /self-tools/server/common/novel_crawler/src/novel.rs
 */
use crate::{NovelSite, chapter::ChapterFn, errors::NovelResult, tag::TagFn};
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum NovelStatus {
    Ongoing,
    Completed,
    Paused,
}
pub trait NovelFn: Sized + Send + Sync {
    type Chapter: ChapterFn;
    type Tag: TagFn;
    const SITE: NovelSite;
    fn get_novel_data(
        novel_id: &str,
    ) -> impl std::future::Future<Output = NovelResult<Self>> + Send;
    fn url(&self) -> String;
    fn name(&self) -> &str;
    fn description(&self) -> &str;
    fn image(&self) -> &str;
    /// The complete catalogue already parsed with this novel, in source order.
    /// Reading it neither fetches data nor copies chapters.
    fn chapters(&self) -> &[Self::Chapter];
    fn author_id(&self) -> &str;
    fn get_url_from_id(id: &str) -> String;
    fn status(&self) -> NovelStatus;
    fn id(&self) -> &str;
    fn tags(&self) -> &[Self::Tag];
}
