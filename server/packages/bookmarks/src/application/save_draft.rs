...
use crate::{
    ...
    application::{author::Author, novel::Novel},
    errors::*,
};
...
pub(super) fn save_tags(
    site: NovelSite,
    tags: &[SaveTagInfo],
    conn: &mut PgConnection,
) -> AppResult<Vec<i64>> {
    let mut existing = TagModel::many_site_id_by_site(site.into(), conn)?;
    let mut result = vec![];
    for tag in tags {
        let id = match existing.get(&tag.id) {
            Some(id) => *id,
            None => {
                let value = TagModel::create(&tag.name, site.into(), &tag.id, conn)?;
...
impl SaveDraftAuthor {
    pub(super) fn save(self, conn: &mut PgConnection) -> AppResult<Author> {
        let mut ids = HashSet::new();
        ...
        super::write(conn, |conn| {
            let author: Author = AuthorModel::create(
                &self.name,
                &self.image,
                self.site.into(),
                &self.id,
            ...
            for novel in &self.novels {
                ...
                let inserted = NewNovel {
                    ...
                    author_id: author.id,
                    novel_status: novel.novel_status.into(),
                    site: self.site.into(),
                    site_id: &novel.id,
                    tags,
                    create_time: now,
...
impl SaveDraftNovel {
    pub(super) fn save(self, conn: &mut PgConnection) -> AppResult<Novel> {
        ...
        super::write(conn, |conn| {
            let now = OffsetDateTime::now_utc();
            let author =
                match AuthorModel::get_id_by_site_id(&self.author.id, self.site.into(), conn)? {
            ...
            let novel = NewNovel {
                ...
                author_id: author.id,
                novel_status: self.novel_status.into(),
                site: self.site.into(),
                site_id: &self.id,
                tags,
                create_time: now,
