...
async fn sql_pagination_batching_and_cost_contract() {
    ...
    diesel::sql_query("TRUNCATE read_record, novel_comment, collection_novel, chapter, novel, author, tag, collection RESTART IDENTITY").execute(&mut c).unwrap();
    ...
    let mut ids = vec![];
    for i in 0..101 {
        ids.push(super::draft(&format!("cost{i}")).save(&mut c).unwrap().id);
    }
    Novel::add_collection(child.id, ids[0], &mut c).unwrap();
    Novel::add_collection(b.id, ids[0], &mut c).unwrap();
    Novel::add_collection(z.id, ids[1], &mut c).unwrap();
    drop(c);
    let app = Application::new(pool.clone(), auth_endpoint());
    count.store(0, Ordering::SeqCst);
    let (first, total) = app
        .query_novels(None, None, None, PageRange::new(1, 20).unwrap())
        .await
        .unwrap();
    assert_eq!(total, 101);
    ...
    let (last, total) = app
        .query_novels(None, None, None, PageRange::new(6, 20).unwrap())
        .await
        .unwrap();
    assert_eq!(total, 101);
    ...
    assert!(
        app.query_novels(None, None, None, PageRange::new(99, 20).unwrap())
            .await
            .unwrap()
            .0
            .is_empty()
    ...
    // SQL array any/all and enum filters share the same count/page predicate.
    let mut c = pool.get().unwrap();
    let extra =
        model::tag::TagModel::create("extra", NovelSite::Jjwxc.into(), "extra", &mut c).unwrap();
    diesel::sql_query("UPDATE novel SET tags=ARRAY[1,$1],novel_status='completed' WHERE id=$2")
        .bind::<diesel::sql_types::BigInt, _>(extra.id)
        .bind::<diesel::sql_types::BigInt, _>(ids[0])
        .execute(&mut c)
        .unwrap();
    drop(c);
    count.store(0, Ordering::SeqCst);
    let (tagged, total) = app
        .query_novels(
            filter(&[a.id], false),
            filter(&[1, extra.id], true),
            Some(NovelStatus::Completed),
            PageRange::new(1, 20).unwrap(),
        )
        .await
        .unwrap();
    assert_eq!(total, 1);
    assert_eq!(tagged[0].id, ids[0]);
    assert_eq!(count.load(Ordering::SeqCst), 4);
    let (_, total) = app
        .query_novels(
            None,
            filter(&[1, extra.id], false),
            Some(NovelStatus::Ongoing),
            PageRange::new(1, 20).unwrap(),
        )
        .await
        .unwrap();
    ...
    for size in [5, 20, 100] {
        count.store(0, Ordering::SeqCst);
        let query = format!(
            "{{queryNovels(pagination:{{page:1,pageSize:{size}}}){{data{{id author{{id name}}}} total}}}}"
        );
        let response = schema
    ...
    count.store(0, Ordering::SeqCst);
    let batch = app.batch_authors((1..=501).collect()).await.unwrap();
    assert_eq!(count.load(Ordering::SeqCst), 2);
    assert_eq!(batch.len(), 1);
    count.store(0, Ordering::SeqCst);
    let response=schema.execute(async_graphql::Request::new(format!("{{getNovel(id:{}){{id wordCount readPercentage chapters{{id isRead author{{id}} novel{{id}}}} firstChapter{{id}} lastChapter{{id}} tags{{id}} collections{{id}} comments{{content}}}}}}",ids[0])).data(router::Auth)).await;
    assert!(response.errors.is_empty(), "{:?}", response.errors);
    let data = response.data.into_json().unwrap();
    assert_eq!(data["getNovel"]["wordCount"], "10");
    assert_eq!(data["getNovel"]["readPercentage"], 0.0);
    assert!(count.load(Ordering::SeqCst) <= 10);
    // Different request must observe a write; NoCache cannot retain old author data.
    let mut c = pool.get().unwrap();
    diesel::sql_query("UPDATE author SET name='changed'")
        .execute(&mut c)
        .unwrap();
    drop(c);
    let response = schema
        .execute(
            async_graphql::Request::new(format!("{{getNovel(id:{}){{author{{name}}}}}}", ids[0]))
                .data(router::Auth),
        )
        .await;
    assert_eq!(
        response.data.into_json().unwrap()["getNovel"]["author"]["name"],
        "changed"
    );
    ...
    let writer = std::thread::spawn(move || {
        ...
        super::draft("concurrent").save(&mut conn).unwrap();
        written_tx.send(()).unwrap();
    });
    let (data, total) = app
        .query_novels(None, None, None, PageRange::new(2, 100).unwrap())
        .await
        .unwrap();
    writer.join().unwrap();
    assert_eq!(total, 101);
