...
fn connection() -> PgConnection {
    let url = std::env::var("BOOKMARKS_TEST_PG").expect("dedicated test database");
    assert!(
        url.split('?')
            .next()
            .unwrap()
...
fn reset(conn: &mut PgConnection) {
    diesel::sql_query("TRUNCATE read_record, novel_comment, collection_novel, chapter, novel, author, tag, collection RESTART IDENTITY").execute(conn).unwrap();
}

fn rename(id: i64, name: &str, conn: &mut PgConnection) -> errors::AppResult<Collection> {
    let parent_id = Collection::get(id, conn)?.parent_id;
    Collection::update(id, name, parent_id, None, conn)
}
...
async fn hierarchy_paths_faults_and_atomic_delete() {
    ...
    let before = state(&mut conn);
    assert!(matches!(
        Collection::update(root.id, "root", Some(child.id), None, &mut conn),
    ...
    let destination = Collection::create("destination", None, None, &mut conn).unwrap();
    let branch = Collection::create("branch", Some(destination.id), None, &mut conn).unwrap();
    model::collection::CollectionModel::set_path(branch.id, "/incorrect-parent-path/", &mut conn)
        .unwrap();
    Collection::update(root.id, "new_%", Some(branch.id), None, &mut conn).unwrap();
    assert_eq!(
        Collection::get(leaf.id, &mut conn).unwrap().path,
        "/destination/branch/new_%/child/leaf/"
    );
    Collection::update(root.id, "new_%", None, None, &mut conn).unwrap();
    assert_eq!(
        Collection::get(leaf.id, &mut conn).unwrap().path,
        "/new_%/child/leaf/"
    );
    assert!(matches!(
        Collection::update(root.id, "root", Some(root.id), None, &mut conn),
        Err(UseCaseError::Rejected(errors::Rejection::Validation(_)))
    ));
    assert!(matches!(
        Collection::update(root.id, "root", Some(999999), None, &mut conn),
        Err(UseCaseError::Rejected(errors::Rejection::Missing(_)))
    ));
    ...
    let entity = draft("retained").save(&mut conn).unwrap();
    Novel::add_collection(last, entity.id, &mut conn).unwrap();
    let before = state(&mut conn);
    ...
    assert_eq!(state(&mut conn), before);
    assert!(
        model::collection_novel::CollectionNovelModel::exists(last, entity.id, &mut conn).unwrap()
    );
    ...
    assert!(Novel::get(entity.id, &mut conn).is_ok());
    assert!(
        !model::collection_novel::CollectionNovelModel::exists(last, entity.id, &mut conn).unwrap()
    );
}
...
fn hierarchy_concurrent_writers_preserve_topology() {
    ...
    let a = Collection::create("a", None, None, &mut conn).unwrap().id;
    let b = Collection::create("b", None, None, &mut conn).unwrap().id;
    assert!(matches!(
        ordered_race(
            &mut conn,
            |conn| Collection::update(a, "a", Some(b), None, conn).map(|c| c.id),
            move |conn| Collection::update(b, "b", Some(a), None, conn).map(|c| c.id)
        ),
        Err(UseCaseError::Rejected(errors::Rejection::Validation(_)))
    ));
    assert_eq!(Collection::get(a, &mut conn).unwrap().parent_id, Some(b));
    assert_eq!(Collection::get(b, &mut conn).unwrap().parent_id, None);
    #[derive(QueryableByName)]
    struct Count {
