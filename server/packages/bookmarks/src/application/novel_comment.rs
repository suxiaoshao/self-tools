...
impl NovelComment {
    pub(super) fn create(novel_id: i64, content: &str, conn: &mut PgConnection) -> AppResult<i64> {
        use crate::errors::*;
    ...
    pub(super) fn delete(novel_id: i64, conn: &mut PgConnection) -> AppResult<i64> {
    ...
    pub(super) fn update(novel_id: i64, content: &str, conn: &mut PgConnection) -> AppResult<i64> {
        use crate::errors::*;
        validate_id(novel_id, "novelId")?;
        super::write(conn, |conn| {
            if !NovelCommentModel::exist_by_novel_id(novel_id, conn)? {
