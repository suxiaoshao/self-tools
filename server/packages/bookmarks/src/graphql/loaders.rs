...
loader!(Authors, Arc<application::Author>, batch_authors);
loader!(Novels, Arc<application::Novel>, batch_novels);
loader!(
    AuthorNovels,
    Vec<Arc<application::Novel>>,
    batch_author_novels
);
loader!(NovelTags, Vec<Arc<application::Tag>>, batch_novel_tags);
loader!(
    NovelCollections,
    Vec<Arc<application::Collection>>,
    batch_novel_collections
);
loader!(
    NovelComments,
    Arc<application::NovelComment>,
    batch_novel_comments
);
loader!(Children, Vec<Arc<application::Collection>>, batch_children);
loader!(Chapters, Vec<Arc<application::Chapter>>, batch_chapters);
loader!(
    ChapterEnds,
    application::read::ChapterEnds,
    batch_chapter_ends
);
loader!(Stats, application::read::NovelStats, batch_stats);
...
pub(super) async fn novels(
    ctx: &Context<'_>,
    data: &[Arc<application::Novel>],
    selection: async_graphql::Lookahead<'_>,
) {
    if selection.field("author").exists() {
        prefetch(ctx, data.iter().map(|v| Authors(v.author_id))).await;
    }
    if selection.field("tags").exists() {
        prefetch(ctx, data.iter().map(|v| NovelTags(v.id))).await;
    }
    if selection.field("chapters").exists() {
        prefetch(ctx, data.iter().map(|v| Chapters(v.id))).await;
    }
    if selection.field("collections").exists() {
        prefetch(ctx, data.iter().map(|v| NovelCollections(v.id))).await;
    }
    if selection.field("comments").exists() {
        prefetch(ctx, data.iter().map(|v| NovelComments(v.id))).await;
    }
    if selection.field("wordCount").exists() || selection.field("readPercentage").exists() {
        prefetch(ctx, data.iter().map(|v| Stats(v.id))).await;
    }
    if selection.field("firstChapter").exists() || selection.field("lastChapter").exists() {
        prefetch(ctx, data.iter().map(|v| ChapterEnds(v.id))).await;
    }
}
